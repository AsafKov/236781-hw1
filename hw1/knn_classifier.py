...
class KNNClassifier(object):
    ...
    def train(self, dl_train: DataLoader):
        ...
        n_classes = torch.unique(y_train).size()
        # ========================
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            _, k_closest = dist_matrix[i].topk(k=self.k, largest=False, dim=0)
            y_pred[i] = torch.bincount(self.y_train[k_closest]).argmax()
            # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    fold_size = (int)(np.floor(dataset_len / num_folds))
    folds = []
    for i in range(num_folds):
        start = min(i*fold_size, dataset_len-1)
        end = min((i+1)*fold_size, dataset_len)
        folds += [indices[start:end]]
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            valid_set = Subset(ds_train, folds[j])
            train_set = Subset(ds_train, list(set(indices) - set(folds[j])))

            dl_valid = DataLoader(valid_set, 1)
            dl_train = DataLoader(train_set, 1)
            model.train(dl_train=dl_train)

            x_valid, y_valid = dataloader_utils.flatten(dl_valid)
            predictions = model.predict(x_valid)
            k_accuracies.append(accuracy(y_valid, predictions))
